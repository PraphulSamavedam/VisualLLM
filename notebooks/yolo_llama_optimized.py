...
# captions_data.drop(columns=["Image file"], inplace=True)
# qa_data.drop(columns=["Image ID"], inplace=True)
...
# print(captions_data.columns)
# print(detections_data.columns)
...
print(f"Converting pandas dataframe to Dataset")
dataset = Dataset.from_pandas(data)

def generate_text(example):
    example["prompt"] = f"Based on the image caption (generated by BLIP model): {example['Generated Caption']}, and Object detections (generated by YOLOv5): {example['Generated Detections']}, Answer the question '{example['Question']}' \n Answer: "
    generated_text = llamaPipe(example["prompt"], max_new_tokens=1)[0]['generated_text']
    return {"Generated Answer": generated_text}

print(f"Mapping zero shot rresults")
dataset_zero_shot = dataset.map(generate_text, batched=True)

df = dataset_zero_shot.to_pandas()
df.to_csv("../data/10k_generation_cfg_answers_with_detections_optimized.csv", index=False)

print("Running Zero shot on LLama Zero Shot Performance with 1 tokens")
...
answers = [{
    ...
    'Generated Answer': llamaPipe(f"Based on the image caption (generated by BLIP model): {lookup[instance['Image ID']][1]}, and Object detections (generated by YOLOv5): {lookup[instance['Image ID']][2]}, Answer the question '{instance['Question']}' \n Answer: ", max_new_tokens=1)[0]['generated_text']
} for instance in tqdm(results)]

# Write data to a CSV file
pd.DataFrame(answers).to_csv('10k_generation_cfg_answers_with_detections_optim_original.csv', index=False)
...
# # Write data to a CSV file
# pd.DataFrame(generation_cfg_answers).to_csv('10k_generation_cfg_answers_with_detections.csv', index=False)
...
#     'Generated Answer': llamaPipe(f"Based on the image caption (generated by BLIP model): {lookup[instance['Image ID']][1]} and Object detections (generated by YOLOv5): {lookup[instance['Image ID']][2]}, Answer in a single word the question: '{instance['Question']}'\n Answer: ", max_new_tokens=3)[0]['generated_text']
# } for instance in tqdm(results)]

# # Write data to a CSV file
# pd.DataFrame(generation_cfg_prompt_restriction_answers).to_csv('10k_generation_cfg_prompt_restriction_answers_with_detections.csv', index=False)
...
#     'Generated Answer': llamaPipe( f"Based on the image caption (generated by BLIP model): {lookup[instance['Image ID']][1]} and and Object detections (generated by YOLOv5): {lookup[instance['Image ID']][2]}. \n In plain simple English language without any emoticons or icons or font colors or punctuation marks. I strongly state do not repeat the question, prompt used, disclaimer, explanantion or anything apart from answer, that is just provide the answer in a single word in lowercase, the question: '{instance['Question']}'. Remember if you are unable to answer the question based on the caption provided by BLIP, mention 'NA'.\nAnswer: ", max_new_tokens=3)[0]['generated_text']
# } for instance in tqdm(results)]

# # Write data to a CSV file
# pd.DataFrame(prompt_with_generation_cfg_answers).to_csv('10k_enhanced_prompt_with_generation_cfg_answers_with_detections.csv', index=False)